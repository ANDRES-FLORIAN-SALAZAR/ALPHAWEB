...
def procesar_registro_empresa(request, user):
    ...
    try:
        ...
        logger.error(f"Error creando perfil empresa: {str(e)}")
        user.delete()
        messages.error(request, f'Error al crear el perfil empresarial: {str(e)}')
...
def contrasenas(request):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            elif 'eliminar_contrasena' in request.POST:
...
def ver_documento(request, documento_id):
    ...
    try:
        ...
        messages.error(request, f'Error al abrir el documento: {str(e)}')
        return redirect('empresa:caja_fuerte')
...
def eliminar_documento(request, documento_id):
    ...
    try:
        ...
        return JsonResponse(
            {'success': False, 'message': f'Error al eliminar el documento: {str(e)}'},
            status=500
        )
